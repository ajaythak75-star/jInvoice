...
function AGMReportModal({ meetings, onClose }: { meetings: SocietyMeeting[]; onClose: () => void }) {
  ...
  const reportText = useMemo(() => {
    ...
    // ── Section 4: Compiled resolutions for the FY ───────────────────────────
    ...
    return lines.join("\n");
  }, [agm, prevAGM, allInFY, selectedFY, prevFY]);
...
function SocietyMeetingsTab() {
  ...
  // Meetings matching the type filter, sorted newest first
  const filtered = useMemo(() => {
    let list = [...meetings];
  ...
  // Group filtered meetings by calendar year → month (newest year/month first)
  const byYearMonth = useMemo(() => {
    const yearMap = new Map<number, Map<number, SocietyMeeting[]>>();
    ...
    <div style={{ padding: "20px", maxWidth: 900, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--color-text-secondary)", marginTop: 3 }}>AGM, Special GM, Committee and Society meetings — grouped by year and month</p>
        </div>
            ...
            <div key={year}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", fontWeight: 600 }}>
                  {months.reduce((s, mo) => s + mo.list.length, 0)} meeting{months.reduce((s, mo) => s + mo.list.length, 0) !== 1 ? "s" : ""}
                </span>
                ...
                {months.map(({ month, list }) => (
                  <div key={month}>
                    ...
                    <div style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-tertiary)", textTransform: "uppercase" as const, letterSpacing: "0.07em", marginBottom: 8, paddingLeft: 4 }}>
                      {MONTH_NAMES[month]} · {list.length} meeting{list.length !== 1 ? "s" : ""}
                    </div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {list.map(m => (
                        <MeetingCard key={m.id} m={m} expandedId={expandedId} setExpandedId={setExpandedId} onDelete={handleDelete} />
                      ))}
                    </div>
                  </div>
                ))}
